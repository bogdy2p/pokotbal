...
var CIRCLE_LABEL_FONTNAME = "MontserratBold";
var CIRCLE_LABEL_FONTCOLOR = cc.color(255, 255, 255,255);
var CIRCLE_LABEL_FONTSIZE = 20;
...
//        this.addChild(this.centerTTF, 2, 0);
        var angleUnit = 120 / this._nodeStringArray.length;
        var angle = 15;
        var delay = 0;
        ...
        for (var i = 0; i < this._nodeStringArray.length; i++) {
            ...
            nodeTTF.setOpacity(100);
            var targetPosition = cc.p(Math.cos(angle * Math.PI / 180) * this._radius, Math.sin(angle * Math.PI / 180) * this._radius /2);

            this.addChild(nodeTTF, 1, i + 1);
            nodeTTF.runAction(cc.Sequence.create(cc.DelayTime.create(delay), cc.Spawn.create(cc.FadeIn.create(0.0), cc.MoveTo.create(0.0, targetPosition))));
//            this.expandTTF();
...
//            return;
        var angleUnit = 180 / this._nodeStringArray.length;
        var angle = 15;
        var delay = 0;
        for (var i = 0; i < this._nodeStringArray.length; i++) {
            angle += angleUnit;
...
CircleLabelTTF.create = function (centerString, nodeStringArray, radius) {
    ...
    if (label && label.init(centerString, nodeStringArray, radius))
        return label;
    return null;
};